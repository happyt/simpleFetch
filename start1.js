// Earlier version using xml-js, preferred xml2js library in other file
...
function playerOnTour(p, code) {
    ...
    if (p.tour !== undefined) {
        ...
        if (p.tour._attributes === undefined) {
            ...
            if (p.name.Country._text !== "" && p.tour[0]._attributes.tourCodeUC === code) {
                result = true;
    //            console.log("tour array");
            }
        } else {
            if (p.tour._attributes.tourCodeUC === code) {
                if (p.name.Country._text !== "") {
                    result = true;
                }
            }
        }
        ...
        if (p.name.Country._text !== "" && p._attributes.primaryTourUC === code) {
            result = true;
 //           console.log("no tour");
        }
    }
